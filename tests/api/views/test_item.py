...
from tests.helpers import reverse, assert_dict_in
...
@pytest.fixture()
def item(client):
    response = client.post(reverse('item-list-view'), {"name": "name", "description": "description"}, format="json")
...
def test_item_create_and_print(client):
    response = client.post(reverse('item-list-view'), {"name": "name", "print": True}, format="json")
    assert response.status_code == status.HTTP_201_CREATED


@pytest.mark.django_db
def test_item_create_image(client):
    response = client.post(reverse('item-list-view'), {
        'name': 'item',
        'image': 'sample_image'
    }, format="json")
    assert response.status_code == status.HTTP_201_CREATED
...
def test_update_item(client, item):
    ...
    response = client.put(reverse('item-detail-view', args=(item['upc'],)),
